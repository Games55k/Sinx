...
type MsgHandle struct {
	Apis           map[uint32]siface.IRouter //存放每个MsgId 所对应的处理方法的map属性
	WorkerPoolSize uint32                    //业务工作Worker池的数量
	TaskQueue      []chan siface.IRequest    //Worker负责取任务的消息队列
}
...
func NewMsgHandle() *MsgHandle {
	return &MsgHandle{
		...
		//一个worker对应一个queue
		TaskQueue: make([]chan siface.IRequest, sutils.GlobalObject.WorkerPoolSize),
	}
}
...
func (mh *MsgHandle) StartWorkerPool() {
	...
	for i := 0; i < int(mh.WorkerPoolSize); i++ {
		...
		mh.TaskQueue[i] = make(chan siface.IRequest, sutils.GlobalObject.MaxWorkerTaskLen)
